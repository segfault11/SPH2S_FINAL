...
//------------------------------------------------------------------------------
__global__ void computeCellStartEnd
(
...
)
{
    ...
    float3 xi;
    xi.x = dPositions[3*id + 0];
    xi.y = dPositions[3*id + 1];
    xi.z = dPositions[3*id + 2];
    ...
    dDensities[id] = rhoi;
    dPressures[id] = gConfiguration.BulkModulus*
...
__global__ void computeAccelerationsD (
    ...
    const unsigned int* dBoundaryIDs,
...
)
{
    ...
    xi.z = dPositions[3*id + 2];
    float3 vi;
    vi.x = dVelocities[3*id + 0];
    vi.y = dVelocities[3*id + 1];
    vi.z = dVelocities[3*id + 2];
    float rhoi = dDensities[id];
    float pi = dPressures[id];
    ...
    dAccelerations[3*id + 0] = fi.x/rhoi + bi.x;
    dAccelerations[3*id + 1] = fi.y/rhoi - 9.81f + bi.y;
    dAccelerations[3*id + 2] = fi.z/rhoi + bi.z;
}
//------------------------------------------------------------------------------
    ...
    mBoundaryData(boundaryData, Grid::ComputeNumGridCells(configuration->Grid))
{
    ...
    thrust::sort_by_key(
        ...
        thrust::device_ptr<unsigned int>(mBoundaryData.dHashs + 
            mBoundaryData.Data->NumParticles),
        thrust::device_ptr<unsigned int>(mBoundaryData.Data->dActiveIDs)
    );
    ...
    computeCellStartEnd<<<mBoundaryData.GridDimensions,
        mBoundaryData.BlockDimensions, 
...
void Solver::computeNeighborhoods ()
{
    ...
    thrust::sort_by_key(
        ...
        thrust::device_ptr<unsigned int>(mFluidData.dHashs + 
            mFluidData.Data->NumParticles),
        thrust::device_ptr<unsigned int>(mFluidData.Data->dActiveIDs)
    );
    ...
    computeCellStartEnd<<<mFluidData.GridDimensions, mFluidData.BlockDimensions, 
        mFluidData.SharedMemSize>>>(
        mFluidData.dCellStart,
        mFluidData.dCellEnd,
        mFluidData.dHashs,
        mFluidData.Data->NumParticles
